...
def show_main_dashboard():
    st.title("ScrumBot")   
    uploaded_file = st.file_uploader("Choose a video file or txt file", type=["mp4", "avi", "mov", "mkv", "txt"])
    
    if uploaded_file is not None:
        
        file_extension = Path(uploaded_file.name).suffix.lower()[1:]
        if file_extension not in ["mp4", "avi", "mov", "mkv", "txt"]:
            st.error("Unsupported file format. Please upload an MP4, AVI, MOV, or MKV file.")
        else:
            ...
            endpoint_name='jumpstart-dft-SpeechToTextModel'
